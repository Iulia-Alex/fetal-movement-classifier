"""
Training script v14 — ComplexUNetV14 (widened architecture 32→256) + SignalMSE.

According to the reference paper (arxiv:2506.22457):
  - Architecture: 6→32→64→128→256, bottleneck 512, instance norm, mask, Diag, RoAct
  - Dataset: FS=500Hz, STFT 128×128 (n_fft=256, hop=15, win=100)
  - Loss: SignalMSE only (temporal MSE after iSTFT @ 500Hz)
"""
...
from complex_network_v14 import ComplexUNetV14
from movement_dataset_v14 import (
    MovementECGDatasetV14,
    NFFT, HOP_LENGTH, WIN_LENGTH, WINDOW_FS,
    TARGET_SIZE_F, TARGET_SIZE_T,
)

# ---------------------------------------------------------------------------
DATA_DIR        = '../data/movement_ecg'
MODEL_SAVE_PATH = '../models/movement_CUNet_v14_large.pth'
HISTORY_PATH    = '../models/movement_CUNet_v14_large_history.json'
LOG_DIR         = '../logs'

LEARNING_RATE   = 1e-4
WEIGHT_DECAY    = 1e-5
BATCH_SIZE      = 8     # smaller than v9 (architecture ~4× larger)
MAX_EPOCHS      = 300
PATIENCE        = 20
VAL_SPLIT       = 0.15
NUM_WORKERS     = 2
...
WINDOW_SAMPLES  = WINDOW_FS                        # 1915 @ 500Hz
ORIG_F          = NFFT // 2 + 1                    # 129
ORIG_T          = TARGET_SIZE_T                    # 128 (natural, no resize needed)
...
def signal_mse(pred_spec: torch.Tensor, fecg_time: torch.Tensor) -> torch.Tensor:
    """MSE temporal: interpolate spectrograma la (129, 128) → iSTFT → MSE cu GT."""
    B, C, H, W = pred_spec.shape
    # Restore ORIG_F (129) pierdut prin resize 129→128
    real = F.interpolate(
...
def main():
    ...
    log_path = os.path.join(LOG_DIR, 'movement_CUNet_v14_large.log')
    import sys as _sys
    ...
    dataset = MovementECGDatasetV14(DATA_DIR)
    ...
    model  = ComplexUNetV14(DIMENSION, in_channels=IN_CHANNELS).to(device)
