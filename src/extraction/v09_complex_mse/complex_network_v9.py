...
# ---------------------------------------------------------------------------
# ComplexUNet v9 — same as v7 but with soft mask at output
# ---------------------------------------------------------------------------
class ComplexUNetV9(nn.Module):
    def __init__(self, dimension, in_channels=6):
        ...
        # Output head → produces logits for the mask
    ...
    def forward(self, x):
        x_in = x                     # save original mixture spectrogram
        ...
        # Soft mask: sigmoid(logits) ∈ [0, 1] applied to input
...
if __name__ == '__main__':
    ...
    model = ComplexUNetV9(h * w, in_channels=c).to(device)
    params = sum(p.numel() for p in model.parameters())
    print(f'Parameters: {params / 1e6:.2f} M')
    y = model(x)
    print(f'Output shape: {y.shape}  (should be same as input)')
