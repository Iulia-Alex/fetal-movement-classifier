...
MODEL_SAVE_PATH = '../models/movement_CUNet_v9_mask.pth'
HISTORY_PATH    = '../models/movement_CUNet_v9_mask_history.json'
LOG_DIR         = '../logs'
...
# ---------------------------------------------------------------------------
# Loss: spectral MSE (real + imag)  — ca ComplexMSE din v1
# ---------------------------------------------------------------------------
def complex_mse(pred_spec: torch.Tensor, fecg_spec: torch.Tensor) -> torch.Tensor:
...
# Combined loss
# ---------------------------------------------------------------------------
def composed_loss(pred_spec, fecg_spec, fecg_time):
    return signal_mse(pred_spec, fecg_time) + complex_mse(pred_spec, fecg_spec)
...
def run_epoch(model, loader, optimizer, device, train=True):
    ...
    with ctx:
        for batch_idx, (x, y, y_time) in enumerate(loader):
            ...
            pred = model(x)
            loss = composed_loss(pred, y, y_time)
...
def main():
    ...
    log_path = os.path.join(LOG_DIR, 'movement_CUNet_v9_mask.log')
    import sys as _sys
    ...
    # Model
    model     = ComplexUNetV9(DIMENSION, in_channels=IN_CHANNELS).to(device)
