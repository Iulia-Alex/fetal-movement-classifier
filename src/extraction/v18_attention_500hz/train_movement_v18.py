"""
Training script v18 — ComplexAttentionUNet at 1 kHz.

Compared to v16 (500 Hz, 128×128):
  - 1 kHz pipeline, 4-second windows, STFT (6, 128, 400).
  - Temporal resolution 10 ms/frame → QRS = 8-10 frames (vs 2-3 at 500 Hz).
  - dimension=128*400=51200 (vs 128*128=16384 for v16).
  - No architectural changes — ComplexAttentionUNet from complex_network_v16.py.
  - Loss: SignalMSE identical to v16 (no attention supervision).

Goal of v18: test whether higher temporal resolution resolves QRS reconstruction.
"""
...
from movement_dataset_v18 import (
    MovementECGDatasetV18,
    FS, WINDOW, NFFT, HOP_LENGTH, WIN_LENGTH,
    TARGET_SIZE_F, TARGET_SIZE_T, ORIG_F,
)

# ---------------------------------------------------------------------------
DATA_DIR        = '../data/movement_ecg'
MODEL_SAVE_PATH = '../models/movement_CUNet_v18_500hz.pth'
HISTORY_PATH    = '../models/movement_CUNet_v18_500hz_history.json'
LOG_DIR         = '../logs'
...
def signal_mse(pred_spec: torch.Tensor, fecg_time: torch.Tensor) -> torch.Tensor:
    """
    pred_spec  : (B, C, 128, 400) complex — output model
    fecg_time  : (B, C, 4000) float32     — GT time domain normalizat
    """
    B, C, Fq, T = pred_spec.shape
    # Re-add the dropped frequency bin (zero-pad back to 129 bins)
    zeros     = torch.zeros(B, C, 1, T, dtype=pred_spec.dtype, device=pred_spec.device)
    pred_full = torch.cat([pred_spec, zeros], dim=2)     # (B, C, 129, 400)
...
def main():
    ...
    log_path = os.path.join(LOG_DIR, 'movement_CUNet_v18_500hz.log')
    ...
    print(f'Pipeline: 1 kHz, WINDOW={WINDOW_SAMPLES}, STFT ({TARGET_SIZE_F}×{TARGET_SIZE_T})')
    print(f'DIMENSION: {DIMENSION}  (128×400)')

    dataset = MovementECGDatasetV18(DATA_DIR)
    ...
    print(f'Arhitectura: ComplexAttentionUNet (v16) @ 1 kHz, 128×400 STFT')
