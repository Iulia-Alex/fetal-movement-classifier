"""
Resume 2 — v1 (ComplexUNet 0.59M, SignalMSE) de la epoch 400.

Continues training from the existing checkpoint (ep399 best=0.013776).
Starts with LR=2.5e-5 (matching final LR of resume 1) to avoid overshooting.
Extends MAX_EPOCHS to 600 (200 more epochs beyond resume 1).

Same save paths as v1 original and resume 1:
  models/movement_CUNet_128x400_composed.pth
  models/movement_CUNet_128x400_composed_history.json
"""
...
LEARNING_RATE   = 2.5e-5   # matches final LR of resume 1 (ep396-400)
WEIGHT_DECAY    = 1e-5
BATCH_SIZE      = 32
MAX_EPOCHS      = 600      # extended from 400
PATIENCE        = 20
VAL_SPLIT       = 0.15
NUM_WORKERS     = 2
PRINT_EVERY     = 20
...
def main():
    ...
    log_path = os.path.join(LOG_DIR, 'movement_CUNet_v1_resume.log')
    import sys as _sys
    ...
    if os.path.exists(HISTORY_PATH) and os.path.exists(MODEL_SAVE_PATH):
    ...
    if epochs_done >= MAX_EPOCHS:
        print(f'Already at {epochs_done} epochs >= MAX_EPOCHS={MAX_EPOCHS}. Nothing to do.')
        return
    ...
    for epoch in range(epochs_done, MAX_EPOCHS):
        ...
        vl_loss = run_epoch(model, val_loader,   optimizer, device, train=False)

        elapsed = time.time() - t0
        ep_time = time.time() - t_ep
        print(f'\n  >> Epoch {epoch + 1}: train={tr_loss:.6f}  val={vl_loss:.6f}  '
