...
# ---------------------------------------------------------------------------
# ComplexUNet v13 — v9 + instance normalization in forward pass
# ---------------------------------------------------------------------------
class ComplexUNetV13(nn.Module):
    def __init__(self, dimension, in_channels=6):
        ...
        # Output head → logits for the mask
    ...
    def forward(self, x):
        x_in = x   # save input for masking at output

        # Instance normalization per (B, C): scale = sqrt(mean(|x|^2))
        scale = (x.real.pow(2) + x.imag.pow(2)).mean(dim=(-2, -1), keepdim=True).sqrt().clamp(min=1e-8)
        x = x / scale   # network sees unit-scale inputs

        x = self.diag_in(x)

        # Encoder cu skip connections
        x    = self.conv1(x)
        res1 = self.down1(x)
        ...
        # Soft mask ∈ [0,1] applied to the ORIGINAL input (at real scale)
...
if __name__ == '__main__':
    ...
    model = ComplexUNetV13(h * w, in_channels=c).to(device)
