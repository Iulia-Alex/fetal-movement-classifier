...
OUT_JSON   = _RES + '/final_testdb_f1_v3_fs500.json'

# ACTUAL data rate: 500 Hz native (NOT 1000). Predictions are on the native grid
# of 600000 samples = 1200s @ 500Hz for all models (v1 directly; v16/v17
# decimate→resample_poly back onto the same grid). QRS detection and tolerance
# must be done at fs=500: otherwise min_dist=250smp becomes 500ms>fetal RR (437ms)
# and find_peaks misses ~every other beat (F1 halved).
FS_REAL = 500
...
def _bandpass(sig, fs, lo=PT_BP_LOW, hi=PT_BP_HIGH):
    key = (fs, lo, hi, len(sig))
...
# derivata 5 puncte (Pan-Tompkins standard)
_DERIV_KERNEL = np.array([-1, -2, 0, 2, 1], dtype=np.float64) / 8
...
def pan_tompkins(sig, fs=FS_REAL):
    ...
    # 2. Derivata (accentueaza panta QRS)
    diff = np.convolve(bp, _DERIV_KERNEL, mode='same')

    # 3. Square (all positive, removes biphasic, accentuates large peaks)
    sq = diff ** 2

    # 4. Moving-window integration (smoothing → single blob per QRS)
    win = max(2, int(PT_INT_MS * fs / 1000))
    kernel = np.ones(win, dtype=np.float64) / win
    integrated = np.convolve(sq, kernel, mode='same')

    # 5. Adaptive threshold: 15% of running mean of local maxima (1s window)
    #    Avoids a global threshold that fails on non-stationary signals.
    win1s = int(1.0 * fs)
    local_max = np.array([
        integrated[max(0,i-win1s//2):min(len(integrated),i+win1s//2)].max()
        for i in range(0, len(integrated), win1s//4)
    ])
    # interpolate threshold
    x_knots = np.arange(len(local_max)) * (win1s // 4)
    thresh_arr = np.interp(np.arange(len(integrated)), x_knots, PT_THRESH * local_max)

    # 6. Binarizeaza si gaseste peaks
    min_dist = int(MIN_DIST_MS * fs / 1000)
    above = (integrated > thresh_arr).astype(np.float64)
    peaks, _ = find_peaks(integrated * above, distance=min_dist)
...
def compute_qrs_f1(pred, gt, fs=FS_REAL):
    tol = int(TOLERANCE_MS * fs / 1000)
    n   = min(pred.shape[1], gt.shape[1])
    results = []
    for ch in range(pred.shape[0]):
        p_peaks = pan_tompkins(pred[ch, :n], fs)
        g_peaks = pan_tompkins(gt[ch,   :n], fs)
...
for si, sig in enumerate(signals):
    ...
    for ver, model, infer_fn in [
        ...
        try:
            ...
            print(f'  {ver}: F1={m["f1"]:.4f}  peaks_pred={m["n_pred_peaks"]:.0f}'
                  f'  peaks_gt={m["n_gt_peaks"]:.0f}', flush=True)
        except Exception as e:
...
# ── raport final ──────────────────────────────────────────────────────────────
print('\n' + '='*70, flush=True)
print(f'FINAL REPORT — QRS F1 Score v3 (Pan-Tompkins) @ fs={FS_REAL}Hz (corrected)', flush=True)
print('='*70, flush=True)
...
for cat in ['all', 'easy', 'medium', 'difficult']:
    ...
    for ver in ['v1', 'v16', 'v17']:
        ...
        if not d['f1']:
            continue
        print(f"  {ver}: F1={np.mean(d['f1']):.4f}±{np.std(d['f1']):.4f}"
              f"  Prec={np.mean(d['precision']):.4f}  Rec={np.mean(d['recall']):.4f}")
