...
OUT_JSON   = _RES + '/final_testdb_f1_v2.json'
...
BANDPASS_LOW  = 3
BANDPASS_HIGH = 40
WIN_THRESH_S  = 5    # window for local threshold computation (seconds)
...
# ── QRS detection with LOCAL threshold ───────────────────────────────────────
_bp_cache = {}
...
def detect_peaks_local(sig, fs=FS1k):
    """Detect R-peaks with a LOCAL threshold per WIN_THRESH_S-second window."""
    filt     = _bandpass(sig.astype(np.float64), fs)
    min_dist = int(MIN_DIST_MS * fs / 1000)
    win_samp = int(WIN_THRESH_S * fs)
    n        = len(filt)

    # compute local threshold: std over current window, sliding by win_samp/2
    local_std = np.empty(n)
    half = win_samp // 2
    for i in range(0, n, half):
        s = max(0, i - half)
        e = min(n, i + half)
        std_val = np.std(filt[s:e])
        local_std[max(0, i - half // 2):min(n, i + half // 2)] = std_val
    # umple marginile
    ...
    pos, _ = find_peaks( filt, height=thresh_median, distance=min_dist)
    neg, _ = find_peaks(-filt, height=thresh_median, distance=min_dist)
...
def compute_qrs_f1(pred, gt, fs=FS1k):
    ...
    for ch in range(pred.shape[0]):
        p_peaks = detect_peaks_local(pred[ch, :n], fs)
        g_peaks = detect_peaks_local(gt[ch,   :n], fs)
...
# ── raport final ──────────────────────────────────────────────────────────────
print('\n' + '='*70, flush=True)
print('FINAL REPORT — QRS F1 Score v2 (LOCAL threshold)', flush=True)
print('='*70, flush=True)
